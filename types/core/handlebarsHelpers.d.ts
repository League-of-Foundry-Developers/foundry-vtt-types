...
declare class HandlebarsHelpers {
   ...
   * property, otherwise add nothing.
   */
  ...
  /**
   * Translate a provided string key by using the loaded dictionary of
  ...
  /**
   * A string formatting helper to display a number with a certain fixed number
   * of decimals and an explicit sign.
   */
  static numberFormat (
    value: any,
    options: HandlebarsHelpers.NumberFormatOptions
  ): string

  /**
   * A helper to create a set of radio checkbox input elements in a named set.
   * The provided keys are the possible radio values while the provided values
   * are human readable labels.
   * @param name - The radio checkbox field name
   * @param choices - A mapping of radio checkbox values to human readable
   *                  labels
  ...
  /**
  * A helper to assign an <option> within a <select> block as selected based on
  * its value
  ...
  */
  static select (
    selected: string,
    options: { fn: Handlebars.TemplateDelegate }
  ): Handlebars.SafeString

  /**
   * A helper to create a set of <option> elements in a <select> block based on
   * a provided dictionary.
   * The provided keys are the option values while the provided values are human
   * readable labels.
   * This helper supports both single-select as well as multi-select input
   * fields.
