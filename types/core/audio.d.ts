 ...
 *
 * Audio playback in Foundry VTT is managed by Howler.js
 * (https://howlerjs.com/). Several methods and attributes in this API return
 * :class:`Howl` instances. See the Howler documentation for details and example
 * usage of the Howl API.
 */
declare class AudioHelper {
  /**
   * The Native interval for the AudioHelper to analyse audio levels from
   * streams Any interval passed to startLevelReports() would need to be a
   * multiple of this value. This number is specified in milliseconds.
   * @defaultValue `50`
   */
   ...
   *              (default: `null`)
   * @param volume - The volume level at which to play the audio, between 0 and
   *                 1
   *                 (default: `1.0`)
   ...
   * this is meant to optimize things and avoid having multiple timers running
   * if we want to analyse multiple streams at the same time.
   * I don't know if it actually helps much with performance but it's expected
   * that limiting the number of timers running at the same time is good
   * practice and with JS itself, there's a potential for a timer congestion
