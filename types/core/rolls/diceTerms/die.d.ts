...
declare class Die extends DiceTerm {
   ...
   * Failures are counted relative to some target, or relative to the lowest
   * possible value if no target is given.
   * Applying a count-failures modifier to the results re-casts all results to 1
   * (failure) or 0 (non-failure)
   ...
   * set.
   * Successes are counted relative to some target, or relative to the maximum
   * possible value if no target is given.
   * Applying a count-success modifier to the results re-casts all results to 1
   ...
   * as -1
   * Failures are identified relative to some target, or relative to the lowest
   * possible value if no target is given.
   * Applying a deduct-failures modifier to the results counts all failed
   * results as -1.
   * 6d6df      Subtract the number of dice which rolled a 1 from the non-failed
   *           total.
   * 6d6cs\>3df  Subtract the number of dice which rolled a 3 or less from the
   *           non-failed count.
   * 6d6cf\<3df  Subtract the number of dice which rolled less than 3 from the
   *           non-failed count.
   * @param modifier - The matched modifier query
   */
   ...
   * Explosion can be a "small explode" using a lower-case x or a "big explode"
   * using an upper-case "X"
   * @param modifier - The matched modifier query
   * @param options - (default: `{}`)
   * @param recursive - Explode recursively, such that new rolls can also
   *                    explode?
   *                    (default: `true`)
   */
   ...
   * failure counts as its negative value.
   * Failures are identified relative to some target, or relative to the lowest
   * possible value if no target is given.
   * Applying a deduct-failures modifier to the results counts all failed
   * results as -1.
