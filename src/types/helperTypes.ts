 ...
 */
export type PromisedType<T> = T extends Promise<infer TResult> ? TResult : T;

/**
 * If `ReturnType<T>` is `Promise<TResult>` then `TResult`; otherwise `ReturnType<T>`.
 */
export type PromisedReturnType<T extends (...args: any) => any> = ReturnType<T> extends Promise<infer TResult>
  ? TResult
  : ReturnType<T>;
