...
declare global {
  ...
  abstract class DocumentCollection<
    T extends foundry.abstract.Document.Constructor,
    Name extends string,
  > extends foundry.utils.Collection<StoredDocument<InstanceType<ConfiguredDocumentClass<T>>>> {
    constructor();
  ...
  namespace DocumentCollection {
    namespace RenderContext {
      interface Base<T extends foundry.abstract.Document.Constructor> {
      ...
      interface Create<T extends foundry.abstract.Document.Constructor> extends Base<T> {
        action: "create";
        data: (InstanceType<T>["data"]["_source"] & { _id: string })[];
      }

      interface Update<T extends foundry.abstract.Document.Constructor> extends Base<T> {
        action: "update";
        data: (DeepPartial<InstanceType<T>["data"]["_source"]> & { _id: string })[];
      }

      interface Delete<T extends foundry.abstract.Document.Constructor> extends Base<T> {
        action: "delete";
        data: string[];
      }
    }
