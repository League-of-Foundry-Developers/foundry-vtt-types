...
declare class MeasuredTemplate extends PlaceableObject<MeasuredTemplateDocument.Implementation> {
   ...
   * @remarks Only `undefined` prior to {@linkcode MeasuredTemplate._refreshShape | MeasuredTemplate#_refreshShape} being called.
   *
   * @privateRemarks Foundry also types this as `| PIXI.Ellipse | PIXI.RoundedRectangle`, but
   * {@linkcode MeasuredTemplate._computeShape | MeasuredTemplate#_computeShape} never returns either in v13
   */
   ...
   * @defaultValue `null`
   * @remarks  Set `null` if the template's document has no {@linkcode MeasuredTemplate.texture | texture} set
   */
   ...
   * @remarks Will return a {@linkcode PIXI.Circle} unless the core `gridTemplates` setting is truthy,
   * in which case it returns a {@linkcode PIXI.Polygon} with points generated by {@linkcode BaseGrid.getCircle | canvas.grid.getCircle}
   * @deprecated "`MeasuredTemplate.getCircleShape` is deprecated. Use {@linkcode foundry.data.CircleShapeData | CircleShapeData}
   ...
   * @remarks Returns a {@linkcode PIXI.Circle} if the core `coneTemplateType` setting is `"round"` and `angle >= 360`,
   * otherwise a {@linkcode PIXI.Polygon}. If the core `gridTemplates` setting is truthy, it always returns a
   * {@linkcode PIXI.Polygon} whose points come from {@linkcode BaseGrid.getCone | canvas.grid.getCone}
   * @deprecated "`MeasuredTemplate.getConeShape` is deprecated. Use {@linkcode foundry.data.ConeShapeData | ConeShapeData}
   * instead." (since v14, until v16)
   */
   ...
   * @remarks Always returns a {@linkcode PIXI.Rectangle}, but if the core `gridTemplates` setting is truthy,
   * the height and width will come from {@linkcode BaseGrid.getTranslatedPoint | canvas.grid.getTranslatedPoint}
   * @deprecated "`MeasuredTemplate.getRectShape` is deprecated. Use
   * {@linkcode foundry.data.RectangleShapeData | RectangleShapeData} instead." (since v14, until v16)
   */
   ...
   * @deprecated "`MeasuredTemplate.getRayShape` is deprecated. Use {@linkcode foundry.data.LineShapeData | LineShapeData}
   * instead." (since v14, until v16)
   */
  ...
  protected override _onUpdate(
    ...
    options: MeasuredTemplateDocument.Database.OnUpdateOptions,
    userId: string,
  ): void;
