...
declare global {
  namespace PIXI {
    interface Rectangle {
       ...
       * @privateRemarks Foundry's `@returns` type says `Point[]`, but its own description documents the `t0` and
       * `t1` properties the returned objects carry. `t1` is only computed on the collinear-with-an-edge path; the
       * general path forwards {@linkcode foundry.utils.lineLineIntersection | lineLineIntersection} results, which
       * leave it `undefined`
       */
      segmentIntersections(a: Canvas.Point, b: Canvas.Point): LineIntersection[];
       ...
       * @privateRemarks Foundry's `@returns` type says `ClipperPoint[]`; the runtime forwards to
       * {@linkcode PIXI.Polygon.intersectClipper | PIXI.Polygon#intersectClipper} and so yields that method's
       * {@linkcode ClipperLib.Paths} solution, or `[]` for a zero-area rectangle
       */
      intersectClipper(
    ...
    namespace Rectangle {
      ...
      interface ClipperLibIntersectPolygonOptions {
        /**
         * A scaling factor passed to {@linkcode PIXI.Polygon.toClipperPoints | Polygon#toClipperPoints} for precision
