...
import type { LineCircleIntersectionPoints } from "#common/utils/geometry.d.mts";
...
declare global {
  namespace PIXI {
    interface Circle {
       ...
       */
      segmentIntersections(a: Canvas.Point, b: Canvas.Point): LineCircleIntersectionPoints;
       ...
       * @param options       - Options which configure how the intersection is computed
       * @returns The intersected polygon
       * @privateRemarks Foundry's description says "polygon" and its `@returns` type says `ClipperPoint[]`; the runtime
       * forwards to {@linkcode PIXI.Polygon.intersectClipper | PIXI.Polygon#intersectClipper} and so yields that
       * method's {@linkcode ClipperLib.Paths} solution, or `[]` for a zero radius
       */
      intersectClipper(
    ...
    namespace Circle {
       ...
       * @privateRemarks Includes `density` because {@linkcode WeilerAthertonClipper.combine} collects its remaining
       * options into `clipOpts` and forwards them to {@linkcode PIXI.Circle.pointsBetween | Circle#pointsBetween},
       * which reads it
       */
      interface WACIntersectPolygonOptions
        extends PIXI.Rectangle.WACIntersectPolygonOptions, InexactPartial<_DensityOptions> {}
      ...
      interface ClipperLibIntersectPolygonOptions
        extends PIXI.Rectangle.ClipperLibIntersectPolygonOptions, InexactPartial<_DensityOptions> {}

      /** @internal */
      interface _DensityOptions {
        /**
      ...
      /** @internal */
      interface _PointsForArcOptions extends _DensityOptions {
        /**
      ...
      interface IntersectClipperOptions extends PIXI.Polygon.IntersectClipperOptions, InexactPartial<_DensityOptions> {}
    }
