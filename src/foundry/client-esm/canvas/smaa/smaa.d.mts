...
declare namespace SMAAFilter {
  interface SMAAFilterConfig {
     ...
     * This allows to eliminate spurious crossing edges, and is based on the fact that, if there is too much contrast in a direction, that will
     *   hide perceptually contrast in the other neighbors.
     */
    localContrastAdaptionFactor: number;

    /**
     * Specifies the maximum steps performed in the horizontal/vertical pattern searches, at each side of the pixel. In number of pixels, it's
     *   actually the double. So the maximum line length perfectly handled by, for example 16, is 64 (by perfectly, we meant that longer lines
     *   won't look as good, but still antialiased. Range: [0, 112].
     */
    maxSearchSteps: number;

    /**
     * Specifies the maximum steps performed in the diagonal pattern searches, at each side of the pixel. In this case we jump one pixel at time,
     *   instead of two. Range: [0, 20].
     */
