...
// This module installs its exports onto `Array.prototype` and `Array`; the augmented members carry the
// documentation, and these declarations describe the same functions as they are reachable by direct import.
// Each is called with `this` bound to the array, except `fromRange`, which is assigned to the constructor.

/**
 * Flatten nested arrays by concatenating their contents
 * @returns An array containing the concatenated inner values
 */
export function deepFlatten<T>(this: T[]): Array<Array.Flattened<T>>;
 ...
 */
export function equals<T, const O>(this: T[], other: OverlapsWith<O, T>[]): boolean;
...
export function partition<T, S extends T>(
  this: T[],
  /** @immediate */ rule: (val: T) => val is S,
): [Exclude<T, S>[], S[]];
export function partition<T>(this: T[], /** @immediate */ rule: (val: T) => boolean): [T[], T[]];
 ...
 */
export function filterJoin<T>(this: T[], sep: string): string;
...
export function findSplice<T>(
  this: T[],
  /** @immediate */ find: (value: T, index: number, obj: T[]) => boolean,
  replace?: T,
): T | null;
