...
import GridlessGrid = foundry.grid.GridlessGrid;
import BaseGrid = foundry.grid.BaseGrid;
import Canvas = foundry.canvas.Canvas;

export const minimalConfig = { size: 100 } satisfies BaseGrid.Configuration;
export const fullConfig = {
  ...
  style: "solidLines",
  thickness: 2,
} satisfies BaseGrid.Configuration;
...
describe("HexagonalGrid Tests", () => {
  test("Construction", () => {
    ...
    new GridlessGrid(minimalConfig);
    new GridlessGrid(fullConfig);
    new GridlessGrid(nullishConfig);
  });

  const grid = new GridlessGrid(fullConfig);

  test("Type", () => {
    expectTypeOf(grid.type).toEqualTypeOf<typeof CONST.GRID_TYPES.GRIDLESS>();

    expectTypeOf(grid.isGridless).toEqualTypeOf<true>();
