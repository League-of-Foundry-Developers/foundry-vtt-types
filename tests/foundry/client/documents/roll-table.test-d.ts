import { expectTypeOf, test } from "vitest";

declare const folder: Folder.Stored;

declare const someTable: RollTable.Stored;

test("foundry/client/documents/roll-table", async () => {
  // @ts-expect-error A RollTable requires data.
  new RollTable.implementation();

  // @ts-expect-error A RollTable requires a name.
  new RollTable.implementation({});

  const table = new RollTable.implementation({ name: "" });

  expectTypeOf(table.results.get("testing")).toEqualTypeOf<TableResult.Stored | undefined>();

  expectTypeOf(await table.draw()).toEqualTypeOf<RollTable.Draw>();
  expectTypeOf((await table.roll()).results).toEqualTypeOf<TableResult.Stored[]>();
  expectTypeOf(table.displayRoll).toEqualTypeOf<boolean>();
  expectTypeOf(await RollTable.fromFolder(folder)).toEqualTypeOf<RollTable.Stored | undefined>();

  expectTypeOf(someTable.prepareDerivedData()).toEqualTypeOf<void>();
  expectTypeOf(someTable.draw({ messageMode: "gm" })).toEqualTypeOf<Promise<RollTable.Draw>>();
  expectTypeOf(someTable.drawMany(2, { messageMode: "self" })).toEqualTypeOf<Promise<RollTable.Draw>>();
});
