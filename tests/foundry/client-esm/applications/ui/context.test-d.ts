...
expectTypeOf(menuA.menuItems[0]?.callback(jquery)).toBeVoid();
...
expectTypeOf(menuB.menuItems[0]?.callback(htmlElement)).toBeVoid();
...
// @ts-expect-error Callback for jQuery: true takes JQuery
expectTypeOf(menuC.menuItems[0]?.callback(htmlElement)).toBeVoid();
